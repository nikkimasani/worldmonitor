...
All paid tiers share the same MCP server, the same 39 tools, and the same OAuth flow — they differ only in **daily quotas**. Pro starts at **50 quota-consuming calls/day**; API Starter and API Business have generous daily allowances; Enterprise is uncapped. See the [Plans & limits](#plans--limits) table below. (`describe_tool`, the v1.5.0 metadata-lookup helper, is exempt from the daily quota.)
...
| Endpoint | Purpose |
|----------|---------|
| `https://worldmonitor.app/mcp` | JSON-RPC server (Streamable HTTP transport; `initialize` defaults to protocol `2025-03-26`) |
